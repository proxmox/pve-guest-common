...
sub storage_migrate {
    ...
    my (undef, $name, $owner, undef, undef, undef, $format) =
    ...
    my $migration_snapshot =
        PVE::Storage::storage_migrate_snapshot($storecfg, $sid, $with_snapshots);
    ...
    my @export_formats =
        PVE::Storage::volume_export_formats($storecfg, $volid, $snapshot, undef, $with_snapshots);
...
our $cmd_schema = {
    bwlimit => {
        ...
        operation => {
            description =>
                'Operation for which bwlimit is queried ("restore", "migration", "clone", "move")',
            type => 'string',
            default => 'migration',
            optional => 1,
        },
    ...
    'disk-import' => {
        ...
        format => PVE::JSONSchema::get_standard_option(
        ...
        migration_snapshot => {
            ...
            optional => 1,
            description =>
                '`snapshot` was created for migration and will be removed after import',
        },
        with_snapshots => {
...
sub handle_disk_import {
    ...
    my $import = PVE::Storage::volume_import_start(
        $state->{storecfg}, $storage, $volname, $format, $state->{vmid}, $params,
    );
