    ...
    my $purge_state = sub {
        ...
        foreach my $vmid (keys %$stateobj) {
            foreach my $tid (keys %{ $stateobj->{$vmid} }) {
                $next_stateobj->{$vmid}->{$tid} = $stateobj->{$vmid}->{$tid}
                    if $used_tids->{$vmid}->{$tid};
            }
        }
        ...
        foreach my $jobid (sort keys %{ $cfg->{ids} }) {
            ...
            if ($jobcfg->{remove_job}) {
                ...
                if (my $fail_count = $state->{fail_count}) {
                    ...
                    if (
                        !$fail_count || ($members->{$target} && $members->{$target}->{online})
                    ) {
                        $next_sync =
                            $state->{last_try} + 60 * ($fail_count <= 3 ? 5 * $fail_count : 30);
                    }
                } else {
                    my $schedule = $jobcfg->{schedule} || '*/15';
                    my $calspec = PVE::CalendarEvent::parse_calendar_event($schedule);
                    $next_sync =
                        PVE::CalendarEvent::compute_next_event($calspec, $state->{last_try}) // 0;
                }
            }
...
sub get_next_job {
    ...
    my $sort_func = sub {
        ...
        my $sb = $jobb->{state};
        return
            $sa->{last_iteration} <=> $sb->{last_iteration}
