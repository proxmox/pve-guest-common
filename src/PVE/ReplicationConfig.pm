...
PVE::JSONSchema::register_format('pve-replication-job-id', \&parse_replication_job_id);
...
sub parse_config {
    ...
    foreach my $id (sort keys %{ $cfg->{ids} }) {
        ...
        if (defined($target_hash->{$vmid}->{$tid})) {
            warn
                "delete job $id: replication job for guest '$vmid' to target '$tid' already exists\n";
            delete $cfg->{ids}->{$id};
        }
