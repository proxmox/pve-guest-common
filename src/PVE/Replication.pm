...
sub replicate_volume {
    my ($ssh_info, $storecfg, $volid, $base_snapshot, $sync_snapname, $rate, $insecure, $logfunc) =
        @_;
...
sub replicate {
    ...
    my $sync_snapname = PVE::ReplicationState::replication_snapshot_name($jobid, $start_time);
...
my $run_replication_nolock = sub {
    ...
    eval { $volumes = replicate($guest_class, $jobcfg, $state, $start_time, $logfunc_wrapper); };
    my $err = $@;
